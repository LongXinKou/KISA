...
class LIV(nn.Module):
    ...
    def forward_video_text(self, videos, text):
        ''''
        input:
            videos:(b,t,c,h,w)
            videos_length:(b,)
            input_sentencelist:(b,)
        output:
            video_embedding:(b,t,c)
            text_embdedding:(b,c)
        '''
        t= videos.shape[1]
        # encode text
        tFeature = self.get_text_feature(text) #(b,1024)
    ...
    def get_frames_feature(self, img: torch.Tensor) -> torch.Tensor:
        '''
        input:
