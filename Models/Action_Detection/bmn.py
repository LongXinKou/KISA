...
class BMN(nn.Module):
    ...
    def forward(self, x):
        '''
        input:
            x:(D,feature_dim=256,T)
        output:
            confidence_map:(D,D,T,T)
            start:(D,T)
            end:(D,T)
        '''
        base_feature = self.x_1d_b(x) #(d,256,T)
        start = self.x_1d_s(base_feature).squeeze(1) #(d,T)
        end = self.x_1d_e(base_feature).squeeze(1) #(d,T)
