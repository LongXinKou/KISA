...
def evaluate(args, model, skill_library):
    ...
    for seed in tqdm(range(100), desc='test'):
        ...
        if flat_prediction[keyframe_idx[-1]] == flat_truth[keyframe_idx[-1]] and keyframe_idx[-1] not in pred_keyframe_index.tolist(): # dim
...
def evaluate_kts(args, model, skill_library):
    f1_score_list, precision_list, recall_list = [], [], []
    for seed in tqdm(range(100), desc='test'):
        frame, label, caption, keyframe_idx = load_data(args.test_path, seed=seed)
        frame = frame.to(args.device) #tensor(bs,t,3,h,w)
        label = label.to(args.device) #tensor(bs,t,1)
        ...
        # KTS
        vFeature, cFeature = model(frame, caption=caption) #tensor(b,t,c)/(b,t,c)
        unique_values, unique_indices = torch.unique(label, return_inverse=True)
        vFeature_array = vFeature.detach().cpu().numpy().squeeze() # array(len, feature_dim)
        cps = segment(vFeature_array, unique_values.shape[0], mode='kts') # array(k)/array(t)
        pred_keyframe_index = torch.tensor(cps).to(args.device) #tensor(k)
    ...
    avg_mae = np.mean(mae_list)
    avg_far = np.mean(far_list)
    return avg_nacc, avg_f1_score, avg_mae, avg_far

def evaluate_uvd(args):
    f1_score_list, precision_list, recall_list = [], [], []
    mae_list, far_list, nacc_list = [], [], []
    for seed in tqdm(range(100), desc='test'):
        frame, label, caption, keyframe_idx = load_data(args.test_path, seed=seed)
        ...
        # uvd
        video_array = torch.squeeze(frame, dim=0).detach().cpu().numpy() #array(t,3,h,w)
        pred_keyframe_index = uvd.get_uvd_subgoals(
            video_array,   # video filename or (L, *video frame shape) video numpy array
            preprocessor_name="vip",    # Literal["vip", "r3m", "liv", "clip", "vc1", "dinov2"]
            device=args.device,  # device for loading frozen preprocessor
            return_indices=True,   # True if only want the list of subgoal timesteps
        )
        pred_keyframe_index = torch.tensor(pred_keyframe_index).to(args.device)
...
if __name__ == '__main__':  
    ...
    for i in range(len(seed_list)):
        ...
        if args.visual_representation == 'uvd':
            # avg_nacc, avg_f1_score, avg_mae, avg_far = evaluate_uvd(args)
            pass
        elif args.visual_representation == 'kts':
            model = load_vlm(args, visual_representation='r3m', pretrain=True).to(args.device)
            model.eval()
            avg_nacc, avg_f1_score, avg_mae, avg_far = evaluate_kts(args, model, skill_library)
        else:
            ...
            model.eval()
            avg_nacc, avg_f1_score, avg_mae, avg_far, avg_iou, avg_t1, avg_t3 = evaluate(args, model, skill_library)
    ...
    if args.visual_representation == 'uvd' or args.visual_representation == 'kts':
        avg_nacc = np.mean(nacc_list)
        var_nacc = np.std(nacc_list)
        print('Number Error:', avg_nacc * 100)
        print('Number Error std:', var_nacc * 100)
